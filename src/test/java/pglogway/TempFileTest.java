 ...
 * tempfile test 
 */
public class TempFileTest extends ScenarioTest {
	...
	public static void kur() throws Exception {
		...
		confDir=new ConfDir(true, econ, "/tmp/tempfile", "mycluster", "5433", 5, new HourList(), new HourList(), 0, 0, 0,
				null, null, null,
	...
	public void denebakalim() throws IOException {
		File dir = new File("/tmp/tempfile");
		FileUtils.deleteDirectory(dir);
		dir.mkdir();
		ExtraFileUtils.copyResourcesRecursively(
				new URL(ExtraFileUtils.class.getResource("/scenarios/tempfile").toString()), new File("/tmp"));

		LogDir ld = new LogDir(confDir, 0, 0);
		ld.run();

		defaultControl("/tmp/tempfile/","postgresql-2021-02-08_10_31_38.csv");
